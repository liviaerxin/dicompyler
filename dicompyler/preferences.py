...
class PreferencesManager:
    ...
    def __init__(
        self, parent, name=None, appname="the application", filename="preferences.txt"
    ):
        ...
        if query[0] in v:
            if query[1] in v[query[0]]:
                for setting, value in list(v[query[0]][query[1]].items()):
                    message = msg + "." + setting
                    pub.sendMessage(
                        message, topic=".".join(["general", setting]), msg=value
                    )
    ...
    def SetPreferenceValue(self, msg):
        ...
        pub.sendMessage(
            list(msg.keys())[0], topic=list(msg.keys())[0], msg=list(msg.values())[0]
        )
    ...
    def CreatePreferencePanel(self, prefpaneldata):
        ...
        if show_restart:
            r = wx.StaticText(
                panel,
                -1,
                "* Restart " + self.appname + " for this setting to take effect.",
...
def main():
    ...
    filename = t.name
    frame.prefmgr = PreferencesManager(
        parent=frame, appname="preftest", filename=filename
    )
