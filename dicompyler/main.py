...
class MainFrame(wx.Frame):
    def __init__(self, parent, id, title, res):
        ...
        self.Bind(wx.EVT_MENU, self.OnPluginManager, id=XRCID("menuPluginManager"))
        ...
        self.Bind(wx.EVT_CHOICE, self.OnStructureSelect, id=XRCID("choiceStructure"))
        ...
        pub.sendMessage(
            "preferences.requested.value", msg="general.advanced.detailed_logging"
        )
        ...
        pub.sendMessage(
            "preferences.requested.value", msg="general.plugins.user_plugins_location"
        )
        pub.sendMessage(
            "preferences.requested.value", msg="general.calculation.dvh_recalc"
        )
        pub.sendMessage(
            "preferences.requested.value", msg="general.plugins.disabled_list"
        )
    ...
    def OnLoadPatientData(self, msg):
        ...
        self.t = threading.Thread(
            target=self.LoadPatientDataThread,
            args=(
                self,
                self.ptdata,
    ...
    def PopulateIsodoses(self, has_images, plan, dose):
        ...
        if has_images and len(plan) and "PixelData" in dose.ds:
            ...
            dosemax = int(
                dosedata["dosemax"]
                * dosedata["dosegridscaling"]
                * 10000
                / plan["rxdose"]
            )
            self.isodoses = [
                {"level": dosemax, "color": wx.Colour(120, 0, 0), "name": "Max"},
                ...
                {"level": 50, "color": wx.Colour(0, 0, 255)},
                {"level": 30, "color": wx.Colour(0, 0, 128)},
            ]
    ...
    def OnUpdatePreferences(self, topic, msg):
        ...
        if (topic[1] == "calculation") and (topic[2] == "dvh_recalc"):
            self.dvhRecalc = msg
        elif (topic[1] == "advanced") and (topic[2] == "detailed_logging"):
    ...
    def OnLicense(self, evt):
        ...
        if guiutil.IsMSWindows():
            ...
            dlg = wx.lib.dialogs.ScrolledMessageDialog(
                self, msg, "dicompyler License", size=(650, 550)
            )
    ...
    def OnActivate(self, evt):
        ...
        if len(sys.argv) == 2:
            ...
            pub.sendMessage(
                "preferences.updated.value", msg={"general.dicom.import_location": path}
            )
            sys.argv.pop()
