...
def pluginProperties():
    ...
    # not qualified version, so that the app will not load this plugin
    props["plugin_version"] = None
...
class pluginDVH(wx.Panel):
    ...
    def Init(self, res):
        ...
        self.Bind(wx.EVT_CHOICE, self.OnToggleConstraints, id=XRCID("choiceConstraint"))
        self.Bind(wx.EVT_SPINCTRL, self.OnChangeConstraint, id=XRCID("txtConstraint"))
        self.Bind(
    ...
    def OnStructureSelect(self, msg):
        ...
        if msg["id"] == None:
            ...
            if self.structureid in self.dvhs:
                ...
                self.dvhscaling[
                    self.structureid
                ] = 1  # self.dvhs[self.structureid]['scaling']
                # 'Toggle' the choice box to refresh the dose data
                self.OnToggleConstraints(None)
            else:
                self.EnableConstraints(False)
                self.guiDVH.Replot(
                    [self.dvharray], [self.dvhscaling], self.checkedstructures
                )
