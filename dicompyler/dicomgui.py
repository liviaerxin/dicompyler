...
class DicomImporterDialog(wx.Dialog):
    ...
    def Init(self, res):
        ...
        self.Bind(
            wx.EVT_TREE_SEL_CHANGED, self.OnSelectTreeItem, id=XRCID("tcPatients")
        )
    ...
    def AddPatientTree(self, patient):
        ...
        if not h in self.patients:
            ...
            self.patients[h]["treeid"] = self.tcPatients.AppendItem(self.root, name, 1)
    ...
    def OnSelectTreeItem(self, evt):
        ...
        if not (self.tcPatients.GetItemData(item) == None):
            ...
            if self.tcPatients.GetItemText(item).startswith(
                "RT Plan"
            ) or self.tcPatients.GetItemText(parent).startswith("RT Plan"):
    ...
    def OnOK(self, evt):
        ...
        if self.tcPatients.GetItemData(item):
            ...
            pub.sendMessage(
                "preferences.updated.value",
                msg={
                    "general.dicom.import_search_subfolders": self.import_search_subfolders
                },
            )
